...
async def watchdog_loop(application: Application, bot_instance: "AegisBot"):
    ...
    Stage 3 — TCP Guard:  conns ≤ 5 (ZOMBIE) for 30s → am force-stop + 2s + rm cache + relaunch
               conns ≥ 8  (ACTIVE) → healthy
    RAM Guard: free RAM < 2 GB → sync
    """
    GRACE_SEC    = 120
    MIN_THREADS  = 145
    TCP_ACTIVE   = 8     # V10.7: healthy (real-world 8-14 conns)
    TCP_ZOMBIE   = 5     # V10.7: ZOMBIE threshold — triggers restart after ZOMBIE_SEC
    ZOMBIE_SEC   = 30    # seconds ZOMBIE state must persist before restart
    POLL_SEC     = 60
    RAM_LOW_GB   = 2.0
        ...
        try:
            ...
            for name, state in list(bot_instance.clone_states.items()):
                ...
                if uptime >= GRACE_SEC:
                    ...
                    if conns <= TCP_ZOMBIE:
                        # ZOMBIE state — restart after ZOMBIE_SEC sustained
                        ...
                        if t0 is None:
                            ...
                            tcp_low_since.pop(name, None)
                            logger.warning(
                                f"Watchdog [{name}]: Stage3 — ZOMBIE "
                                f"TCP={conns} ≤ {TCP_ZOMBIE} for {ZOMBIE_SEC}s"
                            )
                            ...
                            bot_instance.set_state(name, CloneState.STOPPED)
                            asyncio.create_task(
                                bot_instance._enqueue_start(
                                    name,
                                    bot_instance.config.admin_ids[0]
                                    if bot_instance.config.admin_ids else None
                                )
                            )
                            continue
                    else:
...
class AegisBot:
    ...
    async def _hub_view(self) -> Tuple[str, Dict]:
        async def collect_one(clone_name: str) -> Tuple[str, str, str, str, str]:
            ...
            healthy = (
                ...
                and alive
                and thr >= 130
                and conns >= 8   # V10.7: ACTIVE threshold
            )
            return (
                name_disp,
                str(cstate),
    ...
    async def _enqueue_start(self, name: Optional[str], chat_id):
        ...
        if self.clone_states.get(name) == CloneState.RUNNING:
            ...
            if grace_conns < 8:  # V10.7: ACTIVE threshold
                logger.warning(
